...
HEADERS={'User-Agent':'Mozilla/5.0 TransparentaEduRomania/2.0','Accept':'*/*'}
FALLBACK={
 'SB':{'downloadUrl':'https://docs.google.com/spreadsheets/d/1wwlcVOQJ-H70lx8oV3Kdthb07HLEUlTy/export?format=xlsx','format':'xlsx','schoolYear':LEGACY_YEAR},
 'TM':{'downloadUrl':'https://www.isj.tm.edu.ro/public/data_files/media/2026/202603251518-Retea%20scolara%20pentru%20site%202026-2027.pdf','format':'pdf','schoolYear':CURRENT_YEAR,'parser':'timis-pdf'}
}
...
def parse_timisoara_pdf(raw,source_url):
    text='\n'.join((p.extract_text(extraction_mode='layout') or '') for p in PdfReader(io.BytesIO(raw)).pages); schools=[]
...
def registry_sources():
    ...
    for j in r.get('jurisdictions',[]):
        ...
        src=dict(j); src.update({k:v for k,v in FALLBACK.get(j['code'],{}).items() if k not in src})
        urls=src.get('downloads') or ([src.get('downloadUrl')] if src.get('downloadUrl') else [])
        if not urls and src['code'] in FALLBACK:urls=[FALLBACK[src['code']]['downloadUrl']]
        for url in urls:
            x=dict(src); x['url']=url; out.append(x)
    return out,r
...
def parse_source(src):
    ...
    raise RuntimeError(f"Parser {parser or fmt} pentru {src['code']} nu este încă validat; sursa nu este importată pentru a evita date greșite.")
...
out={'schemaVersion':7,'updatedAt':date.today().isoformat(),'targetSchoolYear':CURRENT_YEAR,'coverage':coverage,'importMode':'registry-first','count':len(schools),'currentYearCount':current,'counties':counties,'officialNetworkPages':pages,'sources':sources,'schools':schools}
